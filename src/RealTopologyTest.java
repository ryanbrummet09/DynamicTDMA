...
public class RealTopologyTest {
	...
	public static void main(String args[]) throws IOException {
		 ...
		 * 
		 */
		int numOfSlotsNeededToSendPacket = 2;
		int totalAmountOfTimeToRunSim = 20000;
		...
		int[] flowClasses = {150, 200, 250}; // only has an effect on periodic flows (ie workflow classes 1 and 2)
		int workFlowClass = 3; //options include 1,2,3
        ...
        for(int simNum = 0; simNum < numOfTimesToRepeatSim; simNum++) {
        	...
        	// simple CSMA
        	if(protocol == 1) {
        		simulator = new Simulator(topology, workload, new SimpleCSMAFactory(minContentionWindow), simNum);
        	// exponential CSMA
        	} else if(protocol == 2){
        		simulator = new Simulator(topology, workload, new ExponentialBackoffCSMAFactory(minContentionWindow,maxContentionWindow), simNum);
        	} else {
